(()=>{
  if(globalThis.__notHotdogLoaded)return;globalThis.__notHotdogLoaded=true;
  ...
  function pause(notify=true){enabled=false;current=null;clearTimeout(timer);serial++;bubble.style.display='none';if(notify)chrome.runtime.sendMessage({type:'NOT_HOTDOG_PAUSE'}).catch(()=>{});}
  async function prepare(img){
    if(!img.complete||!img.naturalWidth)throw Error('Wait for the image to finish loading.');
    const bounds=img.getBoundingClientRect();
    if(!img.isConnected||bounds.width<16||bounds.height<16||bounds.x<0||bounds.y<0||bounds.right>innerWidth||bounds.bottom>innerHeight)throw Error('Scroll so the entire image is visible, then hover again.');
    ...
    try{c.getContext('2d').drawImage(img,0,0,c.width,c.height);return {type:'NOT_HOTDOG_CLASSIFY',image:c.toDataURL('image/jpeg',.85)};}catch{}
    ...
    const r=img.getBoundingClientRect();return {type:'NOT_HOTDOG_CAPTURE_CLASSIFY',rect:{x:r.x,y:r.y,width:r.width,height:r.height,viewportWidth:innerWidth,viewportHeight:innerHeight}};
  }
  ...
  document.addEventListener('keydown',e=>{if(e.key==='Escape'&&enabled)pause();},true);
  window.addEventListener('scroll',()=>{bubble.style.display='none';clearTimeout(timer);serial++;current=null;},true);
  chrome.runtime.onMessage.addListener((m,sender,reply)=>{if(m.type==='NOT_HOTDOG_SET_ENABLED'){if(!m.enabled)pause(false);else{enabled=true;current=null;}reply({ok:true});}});
})();
