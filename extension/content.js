(()=>{
  ...
  function pause(){enabled=false;clearTimeout(timer);serial++;bubble.style.display='none';chrome.runtime.sendMessage({type:'NOT_HOTDOG_PAUSE'}).catch(()=>{});}
  ...
  chrome.runtime.onMessage.addListener(m=>{if(m.type==='NOT_HOTDOG_SET_ENABLED'){if(!m.enabled)pause();else{enabled=true;current=null;}}});
})();
