...
export function classifyImage(image){
  if(!worker){worker=new Worker('/device-worker.js?v=0.3.1',{type:'module'});worker.onmessage=({data})=>{const task=pending.get(data.id);if(!task)return;pending.delete(data.id);clearTimeout(task.timer);data.error?task.reject(Error(data.error)):task.resolve(data.result);};worker.onerror=()=>resetEngine();}
