...
export function loadDeviceModel(){
  if(!loading)loading=(async()=>{await tf.setBackend('cpu');await tf.ready();return loadLayersModel(new URL('./model/model.json',import.meta.url).href);})().catch(e=>{loading=null;throw e;});
  return loading;
}
export async function classifyOnDevice(image){
  ...
  ctx.drawImage(bitmap,0,0,224,224);bitmap.close();
  ...
  let output;try{output=model.predict(tensor);return verdictFromScores(await output.data());}finally{tensor.dispose();output?.dispose();}
}
